...
class ContinuousProximalPolicyOptimization(ActorCriticBase):
    ...
    def _actor_loss(self, batch: TransitionBatch) -> torch.Tensor:
        ...
        vs: torch.Tensor = self._critic(state_batch).view(-1)  # shape (batch_size)
        ...
        with torch.no_grad():
            (
                action_batch_old,
                action_batch_log_prob_old,
            ) = self._actor_old.sample_action(state_batch, get_log_prob=True)
        r_thelta = torch.exp(action_batch_log_prob - action_batch_log_prob_old)
        clip = torch.clamp(
        ...
        action_probs = torch.exp(action_batch_log_prob)
        # entropy: H(p) = -sum(p * log(p))
        entropy: torch.Tensor = -torch.sum(action_probs * action_batch_log_prob)
        loss = torch.sum(
